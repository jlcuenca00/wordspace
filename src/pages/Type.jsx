...
export default function Type(){
 ...
 const text=useMemo(()=>{
  ...
  if(cfg.mode==='custom')return cfg.customText.trim()||'Choose custom above, then place any text you want into the room.'
 ...
 const remaining=cfg.mode==='time'?`${Math.max(0,Math.ceil(cfg.time-elapsed))} SEC`:cfg.mode==='words'?`${Math.max(0,cfg.words-typedWords)} WORDS`:`${Math.max(0,text.length-input.length)} CHAR`
 ...
 return <main className={`typeAtelier typeMode-${cfg.mode} controls-${settings.appearance.controls} typed-${settings.behavior.typedText} ${isTyping?'isTyping':''}`} onClick={focusTest}>
  <div className="atelierIdentity" aria-hidden="true"><i>01</i><span>TYPE</span></div>
  <div className="atelierTopline"><span>WORDSPACE / INPUT STUDY</span><span>{new Date().getFullYear()} / SESSION</span><button onClick={e=>{e.stopPropagation();openSettings('caret')}}>CARET / {settings.caret.speed.toUpperCase()}</button></div>

  {!finished&&<TestToolbar locked={isTyping}/>} 

  {!finished?<section className="typingStage">
   <aside className="stageLeft" aria-hidden="true"><span>WORDS</span><i/><span>IN MOTION</span></aside>
   <div className="stageCenter">
    ...
    <div className="typeViewport atelierViewport" style={{'--lines':settings.appearance.lines}}>
    ...
    <div className="atelierStatus"><div><span>PACE</span><b>{startedAt&&settings.appearance.liveWpm?`${wpm} WPM`:'—'}</b></div><div><span>ACCURACY</span><b>{startedAt&&settings.appearance.liveAccuracy?`${accuracy}%`:'—'}</b></div><div><span>REMAINING</span><b>{remaining}</b></div><div><span>RESTART</span><b>{cfg.quickRestart.toUpperCase()}</b></div>{capsLock&&settings.behavior.capsLockWarning&&<div className="statusWarning"><span>KEYBOARD</span><b>CAPS LOCK</b></div>}</div>
   </div>
   <aside className="stageRight"><div><span>MODE</span><b>{cfg.mode.toUpperCase()}</b></div>{!['quote','custom'].includes(cfg.mode)&&<div><span>WORDSET</span><b>{library.label}</b><small>{wordPool.length||'—'} WORDS</small></div>}<div><span>DIFFICULTY</span><b>{cfg.difficulty.toUpperCase()}</b></div><button onClick={e=>{e.stopPropagation();openSettings('test')}}>DEEP SETTINGS ↗</button></aside>
  </section>:
  <section className="atelierResult"><div className="resultAside"><span>RESULT / {failure?'FAILED':'COMPLETE'}</span><i>01</i></div><div className={`result resultV4 ${failure?'failedResult':''}`}><div className="resultKicker"><p>{failure?'SESSION / FAILED':isPb?'SESSION / NEW PERSONAL BEST':'SESSION / COMPLETE'}</p>{failure&&<b>{failure}</b>}</div><strong>{wpm}</strong><h3>WORDS / MINUTE</h3><div className="resultGrid"><span><b>{accuracy}%</b>ACCURACY</span><span><b>{raw}</b>RAW</span><span><b>{consistency}%</b>CONSISTENCY</span><span><b>{incorrect}</b>ERRORS</span></div><ResultGraph samples={samples}/><div className="resultActions"><button onClick={restart}>AGAIN ↗</button><button onClick={practiceWeak}>PRACTICE WEAK KEYS ↗</button><button onClick={()=>navigate('/stats')}>HISTORY ↗</button></div></div></section>}

  <div className="atelierBottom"><span>CLICK TEXT / FOCUS</span><span>CTRL K / COMMAND</span><span>CTRL , / CUSTOMIZE</span></div>
 </main>
}
